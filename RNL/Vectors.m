...
TComponentQ::usage="TComponentQ[\[ScriptE]\[ScriptX]\[ScriptP]\[ScriptR],{\[ScriptU]\[ScriptP]\[ScriptP]\[ScriptE]\[ScriptR]},{\[ScriptL]\[ScriptO]\[ScriptW]\[ScriptE]\[ScriptR]}] gives true if MyTypeOf[\[ScriptE]\[ScriptX]\[ScriptP]\[ScriptR]] matches TComponent[{\[ScriptU]\[ScriptP]\[ScriptP]\[ScriptE]\[ScriptR]},{\[ScriptL]\[ScriptO]\[ScriptW]\[ScriptE]\[ScriptR]},_List]";
...
SupIndex[v_,i_Symbol]/;(MyTypeOf[i]===Untyped&&(AutoDeclareVI/.Options[SupIndex])):=(Declare[i,VectorIndex];SupIndex[v,i])
SubIndex[v_,i_Symbol]/;(MyTypeOf[i]===Untyped&&(AutoDeclareVI/.Options[SubIndex])):=(Declare[i,VectorIndex];SubIndex[v,i])
...
UpperIndices[expr_]:=uli[[1]]/;MatchQ[uli=MyTypeOf[expr],_TComponent];
LowerIndices[expr_]:=uli[[2]]/;MatchQ[uli=MyTypeOf[expr],_TComponent];
Indices[expr_]:=List@@uli[[{1,2}]]/;MatchQ[uli=MyTypeOf[expr],_TComponent];
Dummies[expr_]:=Module[{a=MyTypeOf[Unevaluated[expr]]},If[Head[a]===TComponent,Sort[a[[3]],VISort],{}]];
...
rn[x_,{b__,a_}]:=(Do[If[y=Hold[Unevaluated[x]]/.a->{b}[[j]];MatchQ[MyTypeOf@@y,Number|_TComponent],AppendTo[ri,a];y=y[[1,1]];Break[],y=x],{j,1,Length[{b}]}];rn[y,{b}])
rn[x_,_]:=x;
f={#,","}&;
...
TypeTable[VGrad[h_,a_List,b_List]]:={{{Vector,_}}->Module[{c = (MyTypeOf[h])}, If[MatchQ[c,Function[_Rule|_RuleDelayed]],c=Rule@@c[[1]];If[c == ({{Vector, _}} -> Number ),c= {{Vector, _}} -> TComponent[{}, {}, {}] ];If[MatchQ[c, {{Vector, _}} -> TComponent[_, _, _]],TComponent[Join[c[[2,1]], a], Join[c[[2,2]], b], c[[2,3]]], Badformed],Badformed]],{{Untyped,_}...}->Untyped,_List->Badformed}
VGrad/:HoldPattern[TypeX[gr:(VGrad[h_,a_List,b_List])]]:={Module[{c = (MyTypeOf[h])},If[MatchQ[c,Function[_Rule|_RuleDelayed]],c=Rule@@c[[1]];If[c == ({{Vector, _}} -> Number ),c= {{Vector, _}} ->TComponent[{}, {}, {}] ];If[MatchQ[c, {{Vector, _}} -> TComponent[_, _, _]],Function[Evaluate[{{Vector,_}}->TComponent[Join[c[[2,1]], a], Join[c[[2,2]], b], c[[2,3]]]]], Badformed],Badformed]],Unevaluated[gr]}
